...
def get_input_path(year, month):
    s3_path = os.getenv(
        "INPUT_FILE_PATTERN", f"s3://homework-6/in/{year:04d}-{month:02d}.parquet"
    ).format(year=year, month=month)
    s3 = (
        fs.S3FileSystem(endpoint_override="http://localhost:4566")
        if os.getenv("S3_ENDPOINT_URL")
        else None
    )
...
def get_output_path(year, month):
    output_pattern = os.getenv(
        "OUTPUT_FILE_PATTERN", "s3://homework-6/out/{year:04d}-{month:02d}.parquet"
    )
